...
class MonoEncUNetv0(ResidualEncoderUNet):
    def __init__(self, mono_layer_kwargs: dict = {}, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Extract parameters that shouldn't be passed to encoder
        num_classes = kwargs.pop("num_classes", None)
        ...
        # Extract input_channels from kwargs and pass mono_layer_kwargs as keyword argument
        input_channels = kwargs.pop("input_channels")
        self.encoder = MonoResidualEncoderv0(
            input_channels=input_channels,
            mono_layer_kwargs=mono_layer_kwargs,
            return_skips=True,
            disable_default_stem=False,
...
class MonoEncUNetv1(MonoEncUNet):
    def __init__(self, mono_layer_kwargs: dict = {}, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Extract parameters that shouldn't be passed to encoder
        num_classes = kwargs.pop("num_classes", None)
        ...
        # Extract input_channels from kwargs and pass mono_layer_kwargs as keyword argument
        input_channels = kwargs.pop("input_channels")
        self.encoder = MonoResidualEncoderv1(
            input_channels=input_channels,
            mono_layer_kwargs=mono_layer_kwargs,
            return_skips=True,
            disable_default_stem=False,
...
class MonoEncDecUNet(MonoEncUNet):
    """
...
class MonoEncDecUNetv1(MonoEncUNetv1):
    """
    ResidualEncoderUNet with Monogenic layer at both the encoder front-end and decoder back-end.
    """
    def __init__(self, mono_layer_kwargs: dict = {}, input_channels: int = 1, *args, **kwargs):
...
class MonoResidualEncoderv0(nn.Module):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
                 features_per_stage: Union[int, List[int], Tuple[int, ...]],
    ...
    def forward(self, x):
        ...
        for i, s in enumerate(self.stages):
            if i > 0:
                x = s.blocks[0](x)  # Downsample
                x = x + F.interpolate(lp, size=x.shape[2:], mode='bilinear')  # Residual connection from mono layer
                x = s.blocks[1:](x)
            else:
                x = s(x)
            ret.append(x)
...
class MonoResidualEncoderv1(nn.Module):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
                 features_per_stage: Union[int, List[int], Tuple[int, ...]],
        ...
        for s in range(n_stages):
            ...
            if s > 0:
                # Add a Mono2DV2 layer at each stage of the encoder to compensate for lost capacity
                mono2d = Mono2D(in_channels=input_channels, norm="std", **mono_layer_kwargs)
                stage.blocks.insert(1, mono2d)
...
class MonoUNetDecoder(nn.Module):
    def __init__(self,
        ...
        for s in range(1, n_stages_encoder):
            ...
            mono_layers.append(Mono2D(in_channels=2 * input_features_skip, norm="std", **mono_layer_kwargs))
...
class MonoUNetDecoderv0(nn.Module):
    def __init__(self,
                 mono_layer_kwargs: dict,
                 encoder: Union[PlainConvEncoder, ResidualEncoder],
                 num_classes: int,
    ...
    def forward(self, skips):
        ...
        :return:
        """
        skips, lp = skips  # Unpack skips and local phase features
        lres_input = skips[-1]
        seg_outputs = []
        for s in range(len(self.stages)):
            x = self.transpconvs[s](lres_input)
            x = torch.cat((x, skips[-(s+2)]), 1)
            x = self.stages[s].convs[0](x) + F.interpolate(lp, size=x.shape[2:], mode='nearest')  # Residual connection from mono layer
            x = self.stages[s].convs[1:](x)
            if self.deep_supervision:
                seg_outputs.append(self.seg_layers[s](x))
            elif s == (len(self.stages) - 1):
                seg_outputs.append(self.seg_layers[-1](x))
...
class MonoUNetDecoderv1(nn.Module):
    def __init__(self,
                 mono_layer_kwargs: dict,
                 encoder: Union[PlainConvEncoder, ResidualEncoder],
                 num_classes: int,
        ...
        for s in range(1, n_stages_encoder):
            ...
            # input features to conv is 2x input_features_skip (concat input_features_skip with transpconv output)
            mono2d = Mono2D(in_channels=input_features_skip, norm="std", **mono_layer_kwargs)
            stage.convs[0].all_modules.append(mono2d)
            stages.append(stage)
            # we always build the deep supervision outputs so that we can always load parameters. If we don't do this
            # then a model trained with deep_supervision=True could not easily be loaded at inference time where
            # deep supervision is not needed. It's just a convenience thing
